      ...
      try {
        ...
        if (Array.isArray(data) && data.length > 0) {
          const formatted = data.map((emp: any) => {
            const bio = emp.enrolledBiometrics || emp.biometrics || {
              fingerprint: false,
              face: false,
              rfid: false,
              rfidCard: false,
            ...
            return {
              ...emp,
              id: emp.id?.toString() || emp.employee_id || `EMP-${Date.now()}`,
              employeeId: emp.employee_id || emp.employeeId || emp.id?.toString() || `EMP-${Date.now()}`,
              biometricId: emp.biometricId || emp.biometric_id || `BIO-${emp.id || '8000'}`,
              fullName: emp.full_name || emp.fullName || emp.name || 'Unnamed Employee',
              name: emp.full_name || emp.fullName || emp.name || 'Unnamed Employee',
              email: emp.email || '',
              phone: emp.phone || emp.contact || '',
              departmentId: emp.departmentId || emp.department_id || 'DEP-01',
              ...
              joiningDate: emp.joiningDate || (emp.created_at ? new Date(emp.created_at).toLocaleDateString() : 'Jan 1, 2026'),
              avatarUrl:
                emp.avatarUrl ||
                emp.avatar_url ||
                `https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=150&auto=format&fit=crop&q=80`,
              biometrics: {
                fingerprint: Boolean(bio.fingerprint),
                face: Boolean(bio.face),
                rfid: Boolean(bio.rfid || bio.rfidCard),
                rfidCard: Boolean(bio.rfid || bio.rfidCard),
              },
              enrolledBiometrics: {
                fingerprint: Boolean(bio.fingerprint),
                face: Boolean(bio.face),
...
prev.map((e) => {
  ...
  const workingNowCount = todayAttendance.filter(
    (r) =>
      (r.status === 'Working' || (r.checkIn && !r.checkOut)) &&
  ...
  const simulateBiometricScan = (
    ...
    if (action === 'check_out') {
      ...
      const overtimeMins =
        currentMins > closingMins + settings.attendanceRules.overtimeThresholdMinutes
      ...
      setAttendance((prev) => prev.map((r) => (r.id === existing.id ? updatedRecord : r)));
    ...
    if (action === 'break_end') {
      ...
      setBreaks((prev) => prev.map((b) => (b.id === activeBreak.id ? updatedBreak : b)));
